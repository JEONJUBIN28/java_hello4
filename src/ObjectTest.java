...
//	      A a; //변수선언 int(데이터 타입) num(변수명);
//	      //참조형 = 4바이트(32비트) 8가지 데이터 타입외 선언할때 (클래스 이름) 
//	      //메모리 주소를 한방에 보낼수있는 CPU의 값이 4바이트이기 때문이다, 용도는 주소
//	      a=new A(); //객체(인스턴스)생성 // A(); 생성자 함수 : 클래스 이름과 똑같은 함수.
//	      //메모리에 방을 하나씩 잡아서 다 잡고서 가장 처음의 주소를 객체에 넣어준다 그것이 객체 = 인스턴스 
//	      //(메모리 관리하는것)주소를 정해주는것은 OS이다.
